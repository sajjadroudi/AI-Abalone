...
static func alpha_beta_search(state, max_player):
	var result = max_value(state, 0, max_player, -INF, +INF)
	return result["action"]
	
static func max_value(state, depth, max_player, alpha, beta):
	...
	for item in Successor.calculate_successor(state, max_player):
		var result = min_value(item["state"], depth + 1, max_player, alpha, beta)
...
static func min_value(state, depth, max_player, alpha, beta):
	if CutoffTest.test(state, depth):
		return {"value": Evaluator.eval(state, max_player), "action": null}
	...
	for item in Successor.calculate_successor(state, max_player):
		var result = max_value(item["state"], depth + 1, max_player, alpha, beta)
